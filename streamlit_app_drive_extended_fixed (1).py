...
import requests  # usato per scaricare file da Google Drive senza usare le API Google
...
from Google Drive if no file is uploaded.  The download from Drive is
implemented using a simple HTTP request (via ``requests``) to avoid
reliance on the Google client libraries.
"""
...
def download_drive_file(file_id: str, suffix: str = ".xlsx") -> str:
    """Download a file from Google Drive via HTTP and write it to a temporary file.
