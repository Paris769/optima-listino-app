...
(`streamlit_app_drive_extended_fixed.py`) when deploying your app on
...
# Page configuration must be set before any other Streamlit command.  This
# call ensures that Streamlit knows the title and layout up front.  It must
# remain at the very top of the app logic to avoid the common error
# "st.set_page_config() can only be called once and must be called as the
...
def confirm_column_mapping(supp_df: pd.DataFrame, defaults: Dict[str, List[str]]) -> Dict[str, str]:
    ...
    match is present in the DataFrame columns, it is selected by default.
    ...
    defaults : Dict[str, List[str]]
        A mapping from internal field name to a list of potential column names
        expected in the supplier data.  The first matching element, if present,
        will be used as the default selection.

    Returns
    -------
    Dict[str, str]
        A mapping from internal field name to the actual column name chosen by
        the user.
    """
...
def save_mapping(supplier_name: str, mapping: Dict[str, str]) -> None:
    ...
    The mapping will be saved to ``mappings/<supplier_name>/mapping.json`` relative
    to the current working directory.  The parent directories are created if they
    do not exist.
...
def get_changes_for_confirmation(
    ...
    updater : ListinoUpdater
        The price list updater instance (already initialised with the master listino).
...
# Begin Streamlit UI logic.  Since we've already called st.set_page_config
# above, the rest of the UI can freely use Streamlit commands.
...
if st.button("▶️ Esegui aggiornamento"):
    ...
    for supp_file in suppliers_files:
        ...
        if selected_update:
            if 'codice' in updates_df.columns:
                df_update_sel = updates_df[updates_df['codice'].isin(selected_update)]
            else:
                df_update_sel = updates_df.loc[updates_df.index.astype(str).isin(selected_update)]
        else:
            df_update_sel = pd.DataFrame(columns=supp_df.columns)
        if selected_new:
            if 'codice' in inserts_df.columns:
                df_insert_sel = inserts_df[inserts_df['codice'].isin(selected_new)]
            else:
                df_insert_sel = inserts_df.loc[inserts_df.index.astype(str).isin(selected_new)]
        else:
